...
st.title("hi this is a E-Commerce Dashboard")
...
if fl is not None:
    filename = fl.name
    st.write(filename)
    df = pd.read_csv(filename, encoding='ISO-8859-1') if filename.endswith(('csv','txt')) else pd.read_excel(filename)
else:
    # os.chdir(r"D:\Rupali_Personal\Streamlit_dashboard")
    df = pd.read_csv("Superstore.csv", encoding='ISO-8859-1')

col1, col2 = st.columns((2))
df["Order Date"] = pd.to_datetime(df['Order Date'], dayfirst=True)

# Getting the win and max date
...
with cl1:
    with st.expander("Category_ViewData"):
        ...
        st.download_button("Download Data", data = csv, file_name="Category_data.csv", mime='text/csv', help="Click here to download data as a CSV file")
